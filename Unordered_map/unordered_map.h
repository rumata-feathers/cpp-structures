...
class UnorderedMap {
 ...
 public:
  ...
  UnorderedMap(const UnorderedMap &, const Alloc & = Alloc());
 ...
 private:
  ...
  void swap(UnorderedMap &other_map) {
    std::swap(node_alloc_, other_map.node_alloc_);
    std::swap(fake_node, other_map.fake_node);
    std::swap(key_hash_map, other_map.key_hash_map);
    std::swap(key_comparator, other_map.key_comparator);
    std::swap(map_hasher_, other_map.map_hasher_);
  }
 ...
 public:
  ...
  class NodeIterator {
   public:
    ...
    explicit NodeIterator(BaseNode *new_node) noexcept: node(new_node) {};
...
UnorderedMap<Key, Value, Hash, Equal, Alloc>::UnorderedMap(const Alloc &other_alloc)
...
UnorderedMap<Key, Value, Hash, Equal, Alloc>::UnorderedMap(UnorderedMap &&other_map) noexcept
    : node_alloc_(other_map.node_alloc_) {
  fake_node = std::move(other_map.fake_node);
  key_hash_map = std::move(other_map.key_hash_map);
  map_size = other_map.map_size;
  other_map.key_hash_map.clear();
  other_map.map_size = 0;
}
...
UnorderedMap<Key, Value, Hash, Equal, Alloc> &UnorderedMap<Key,
  ...
  try {
    auto tmp = UnorderedMap(other, std::allocator_traits<node_allocator>::propagate_on_container_copy_assignment::value
                                   ? other.node_alloc_
                                   : node_allocator());
    swap(tmp);
    return *this;
...
typename UnorderedMap<Key, Value, Hash, Equal, Alloc>::mapped_type &UnorderedMap<Key,
  ...
  if (it == end()) {
    return emplace(
        std::piecewise_construct,
        std::tuple<key_type>{k},
        std::tuple<mapped_type>{mapped_type()}
    ).first->second;
  } else {
    return it->second;
  }
...
template<class Key, class Value, class Hash, class Equal, class Alloc>
typename UnorderedMap<Key, Value, Hash, Equal, Alloc>::size_type UnorderedMap<Key, Value, Hash, Equal, Alloc>::
...
typename UnorderedMap<Key, Value, Hash, Equal, Alloc>::size_type UnorderedMap<Key,
                                                                              ...
                                                                              Equal,
                                                                              Alloc>::bucket_size(
    UnorderedMap::size_type n) const {
  try {
...
std::pair<typename UnorderedMap<Key, Value, Hash, Equal, Alloc>::iterator, bool> UnorderedMap<Key,
  ...
  try {
    if (find(obj.first) == end()) {
      iterator ins_it = emplace(obj).first;
...
std::pair<typename UnorderedMap<Key, Value, Hash, Equal, Alloc>::iterator, bool> UnorderedMap<Key,
  ...
  try {
    std::allocator_traits<node_allocator>::construct(node_alloc_, new_node, std::forward<Args>(args)...);

    auto s_it = find(new_node->kv.first);
    if (s_it.get_node() == &fake_node) {
      s_it = iterator(key_hash_map[bucket(new_node->kv.first)]);

      new_node->hash = map_hasher_(new_node->kv.first);
